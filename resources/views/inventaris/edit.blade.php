        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <form action="{{ route('inventaris.update', $inventaris) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="tahun_beli" id="tahun_beli" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('tahun_beli') border-red-500 @enderror" value="{{ old('tahun_beli', $inventaris->tahun_beli) }}" required min="1900" max="{{ date('Y') + 1 }}">
                    @error('tahun_beli')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
